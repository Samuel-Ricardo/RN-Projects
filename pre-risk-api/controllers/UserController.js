...
const Item = require("../models/Pilot");
...
class UserController{
    ...
    async create(req, res) {
        ...
        const { name , email, cnpj , password , number, isAdmin } = req.body;
        ...
        if (userAlreadyExists) {
            ...
            const data = {
                error: true,
                message: default_messages.email_repeated
            }
            ...
            if (password == "" || password == undefined || password == " ") {
                ...
                if (user != undefined) {
                    ...
                    const data = {
                        error: true,
                        message: default_messages.user_create_error
                    }
    ...
    async update(req, res){
        const { userId, name , email , cnpj, number, isAdmin} = req.body;

        const updatedUser = await User.findOneAndUpdate({_id: userId},{name , email, cnpj, number, isAdmin});
    ...
    async login(req, res) {
        ...
        if (user != undefined) {
            ...
            if(passwordHash){
                ...
                res.status(400).send({ data: data2})
            }
        }else{     
            ...
            const data3 = {
                ...
                res.status(400).send({data: data3})
        }
    }
    ...
    async addDocs(req, res){
        ...
        //res.json({userInfo})
        const info = userInfo.docList.map(async (data)=>{
            //if(info.length != 0){
                const unfavoriteItem = await User.updateOne({_id : userId}, {$push : {docList: docId }});//Zera os favoritos de 1 usuário 
                const unfavoritedUser = await Item.updateOne({_id : docId}, {$push : {addPor: userId}});//Zerar o contador de favoritados do item
                return res.json({message:"Item desfavoritado"})
            //}
        })
        
        if(info.length == 0){
            const docList = await User.findOneAndUpdate({_id : userId}, {$push : {docList: docId }});
            const addPor = await Item.findOneAndUpdate({_id: docId},{$push: {addPor: userId }});
    ...
    async getDocs(req, res){
        ...
        const userDocs = userInfo.docList;
        res.json({ userDocs}); 
    }
